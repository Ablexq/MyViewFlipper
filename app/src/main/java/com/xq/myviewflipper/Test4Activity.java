...
public class Test4Activity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        context = this;
        viewFlipper = (ViewFlipper) findViewById(R.id.vf4);
        initData();
        setViews();
    }
    ...
    private void setViews() {
        if (titles.size() > 0) {
            //计算ViewFlipper视图的数目
            int viewNum = titles.size() / 2 + 1;//4页 （01）（23）（45）（6）
            ...
            //视图进入动画
            viewFlipper.setInAnimation(context, R.anim.new_in);
            //视图退出动画
            viewFlipper.setOutAnimation(context, R.anim.new_out);
            //自动开始滚动
